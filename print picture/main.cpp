...
class Picture
{
    ...
    public:
    /// shapes setter
    void setRectangles(Rect* rptr,int rSize)
    {
        rectangleptr = rptr;
        rectangleSize = rSize;
    }
    void setLines(Line* lptr,int lSize)
    {
        lineptr = lptr;
        lineSize = lSize;
    }
    void setTriangles(Triangle* tptr,int tSize)
    {
        triangleptr = tptr;
        triangleSize = tSize;
    }
    void setCircles(Circle* cptr,int cSize)
    {
        circleptr = cptr;
        circleSize = cSize;
    }
    ...
    Picture()
    {
        ...
        triangleptr = NULL;
        circleptr = NULL;
        rectangleSize = lineSize = teriangleSize = circleSize = 0;
    }
    Picture (Rect* rptr,int rSize, Line* lptr,int lSize, Triangle* tptr,int tSize, Circle* cptr,int cSize)
    {
        rectangleptr = rptr;
        rectangleSize = rSize;
    ...
    void Paint()
    {
        ...
        {
            triangleptr[i].DrawTriangle();
        }
        for(int i = 0; i < circleSize; i++)
        {
            circleptr[i].DrawCircle();
        }
    }
};
...
int main()
{
    ...
    Triangle tarr[2]={Triangle(1,2,3,4,6,7,5)};
    Circle carr[4]={Circle(1,2,3,4),Circle(6,7,8,9),Circle(p1, 30, 55), Circle(2,6,4,3)};

    Picture p;
    p.setLines(larr,2);
    p.setRectangles(rarr,3);
    p.setTriangles(tarr,2);
    p.setCircles(carr,4);

    p.Paint();

    p.setLines(NULL,0);
    p.setRectangles(NULL,0);
